...
def process_cat_features(news_df):
    ...
    #update category
    category0_encoder = get_categ_encoder_from_values(news_df['category'].unique())
    print('Category0 - unique count {}'.format(len(category0_encoder)))
    news_df['category0_encoded'] = transform_categorical_column(news_df['category'], category0_encoder)
...
def main():
    ...
    #read file content articale CSV
    ...
    #update news_df['title']
    # print('Tokenizing articles...')
    # tokenized_articles = tokenize_articles(news_df['text_highlights'].values, tokenization_fn=tokenize_norwegian_article)
    print('Tokenizing articles...')
    tokenized_articles = tokenize_articles(news_df['title'].values, tokenization_fn=tokenize_norwegian_article)
